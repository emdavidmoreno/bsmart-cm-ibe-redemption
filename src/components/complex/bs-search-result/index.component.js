  ...
  return {
    ...
    <div class="m-itinerary-summary" data-ng-show="location.departure.summary.show || !location.departure.selectingValueForFirstTime"
      ...
      <div class="itinerary-summary-flight itinerary-summary-flight--departure" data-ng-show="location.departure.summary.show">
            ...
            <div class="l-flight-detail" tabindex="0">
              <div class="flight-detail flight-detail--origin">
                ...
                <span class="time"> {{flight.departure_time | simpledate}}
                  <sup data-ng-repeat="dec in flight.disclaimer" data-ng-if="dec.field == 'departure_time'">{{location.disclaimer_mapping[dec.id].symbol}}</sup>
                </span>
              </div>
              <i class="flight-direction"></i>
              <span class="flight-duration--center" data-ng-if="flight.duration &amp;&amp; flight.duration &gt;= 0">{{flight.duration | duration}}</span>
              <div class="flight-detail flight-detail--destination">
                <span class="location">{{flight.destination_airport_code}}</span>
                <span class="time"> {{flight.arrival_time | simpledate}}
                  <sup data-ng-repeat="dec in flight.disclaimer" data-ng-if="dec.field == 'arrival_time'">{{location.disclaimer_mapping[dec.id].symbol}}</sup>
                </span>
              </div>
            </div>
        ...
        <div class="price" data-ng-if="location.departure.summary.price &gt;= 0">
          <span data-ng-if="$ctrl.ui.total_price.currency_code &gt;= 0">{{$ctrl.ui.total_price.currency_code}}</span>
          <strong class="cash--after-discount" data-ng-if="location.departure.summary.cash_after_discount &gt; 0">
          {{location.departure.summary.cash_after_discount | number:2 }}</strong>
          <strong class="cash" data-ng-class="{'price--deleted': location.departure.summary.cash_after_discount &gt; 0}" data-ng-if="location.departure.summary.price &gt; -1">
              ...
              <div class="m-dialog-item" data-ng-repeat="option in location.departure.availableClasses track by option.name" data-ng-class="{'m-dialog-item-disabled': option.cheapestPrice === 'N/A', 'm-dialog-item-selected': option.selected}">
                ...
                data-ng-click="$ctrl.ui.onSelectFlightClass(option, true, location)">
                  ...
                  <span class="m-item-price">
                    ...
                    <strong class="cash" data-ng-class="{'price--deleted': option.cash_after_discount &gt; 0}"
                      data-ng-if="option.cheapestPrice &gt; -1"> {{option.cheapestPrice | number:2 }}</strong>
                  </span>
                  <ul class="m-item-descriptions" data-ng-if="option.showDesc">
                ...
                <div class="flight-time" tabindex="0">
                  <div data-ng-repeat="fInfo in flight.info.flight_list">
                    <span class="departing">
                      ...
                      <span class="location">{{fInfo.origin_airport_code}}</span>
                    </span>
                    <i class="direction-going"></i>
                    <span class="returning">
                      <strong class="time"> {{flight.info.arrival_time | simpledate}}</strong>
                      <span class="location">{{fInfo.destination_airport_code}}</span>
                    </span>
                    <div class="declaimer">
                  ...
                  <span class="stops" data-ng-if="flight.info.flight_list.length &gt; 1">{{flight.info.flight_list.length - 1}} {{ 'LABEL_FLIGHT_STOPS' | translate }}</span>
    ...
    <div class="m-itinerary-summary" data-ng-show="location.departure.summary.show || !location.departure.selectingValueForFirstTime"
      ...
      <div class="itinerary-summary-flight itinerary-summary-flight--return" data-ng-show="location.return.summary.show">
        ...
        <div tabindex="0" class="price" data-ng-if="location.return.summary.price &gt;= 0">
          ...
          <strong class="cash" data-ng-class="{'price--deleted': location.return.summary.cash_after_discount &gt; 0}" data-ng-if="location.return.summary.price &gt; -1">
        ...
        <div class="m-flight-options m-inbound-flight-options">
          ...
          <div class="dialog-container" data-jqui-dialog="data-jqui-dialog" data-bs-open-dialog="{{$ctrl.ui.returnDialogIsOpen}}" data-bs-close-from-inner="$ctrl.ui.returnDialogIsOpen">
            ...
            <div class="m-dialog-item" data-ng-repeat="option in location.return.availableClasses track by option.name" data-ng-class="{'m-dialog-item-disabled': option.cheapestPrice === 'N/A', 'm-dialog-item-selected': option.name === location.return.selectedClassIndex.name}">
              ...
              <div class="m-item-col-right" data-ng-click="$ctrl.ui.onSelectFlightClass(option, false, location)">
                <span class="m-item-name">{{option.name}}</span>
                <span class="m-item-price">
                  <strong class="cash--after-discount" data-ng-if="option.cash_after_discount &gt; 0"> {{option.cash_after_discount | number:2 }}</strong>
                  <strong class="cash" data-ng-class="{'price--deleted': option.cash_after_discount &gt; 0}"
            ...
            data-ng-click="$ctrl.selectFlightAction(flight, location, 'return', $parent.$index, $event)">
              <div class="flight-time">
                <div data-ng-repeat="fInfo in flight.info.flight_list">
                  ...
                  <i class="direction-going"></i>
                  <span class="returning">
                    <strong class="time"> {{fInfo.arrival_time |simpledate}}</strong>
                    <span class="location">{{fInfo.destination_airport_code}}</span>
                  </span>
                  <div class="declaimer">
                    <span data-ng-repeat="dec in fInfo.disclaimer" data-ng-if="dec.field == 'flight_number'">
                      {{location.disclaimer_mapping[dec.id].text}}
                    </span>
                  </div>
                </div>
              ...
              <div class="flight-price">
                <strong class="cash--after-discount" data-ng-if="flight.info.classes[location.return.selectedClassIndex.id].price.cash_after_discount &gt; 0">{{flight.info.classes[location.return.selectedClassIndex.id].price.cash_after_discount | number:2 }}</strong>
                <strong class="cash" data-ng-class="{'price--deleted': flight.info.classes[location.return.selectedClassIndex.id].price.cash_after_discount &gt; 0}"
                  data-ng-if="flight.info.classes[location.return.selectedClassIndex.id].price.cash &gt; -1">{{flight.info.classes[location.return.selectedClassIndex.id].price.cash | number:2 }}</strong>
                <strong data-ng-if="flight.info.classes[location.return.selectedClassIndex.id].price.cash == -1">N/A</strong>
              </div>
            </div>
